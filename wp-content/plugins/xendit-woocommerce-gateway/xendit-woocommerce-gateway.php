...
Plugin URI: https://www.xendit.co/en/
Description: Xendit Payment Plugin for Woocommerce
Version: 1.0
Author: Joe Contini
Author URI: https://www.xendit.co/en/
*/
    ...
    class woocommerce_xendit_gateway extends WC_Payment_Gateway{
		...
		function init_form_fields(){
            ...
            $this->form_fields = array(
					...
					'api_key' => array(
						'style' => '',
						'class' => 'xendit_live',
						'title' => __('Xendit API key :', 'xendit'),
						'type'=> 'text',
					...
					'callback_token' => array(
						'style' => '',
						'class' => 'xendit_live',
						'title' => __('Xendit Callback token :', 'xendit'),
						'type'=> 'text',
						'description' => __('Unique Live Callback token given by xendit. <strong>Case Sensitive!</strong>', 'xendit'),
						'default' => __('', 'xendit')),

					'api_key_dev' => array(
						'style' => '',
						'class' => 'xendit_dev',
						'title' => __('Xendit API key [DEV] :', 'xendit'),
						'type'=> 'text',
					...
					'callback_token_dev' => array(
						'style' => '',
						'class' => 'xendit_dev',
						'title' => __('Xendit Callback token [DEV] :', 'xendit'),
						'type'=> 'text',
						'description' => __('Unique Development Callback token given by xendit. <strong>Case Sensitive!</strong>', 'xendit'),
						'default' => __('', 'xendit')),
					...
					'responce_url_calback' => array(
						...
						'default' => home_url().'/thank-you-page/?xendit_mode=xendit_callback'),
       ...
       public function payment_fields(){
		...
		public function confirmation_email($order_id){
			...
			$this->buat_log("{$this->xendit_status} Order Created: Xendit sent response..", true);
			if($this->developmentmode == 'yes')
				$this->buat_log(json_encode($response, JSON_PRETTY_PRINT), true);
			
			if(isset($response['error_code'])){
			
				update_post_meta( $order_id, 'Xendit_error', esc_attr($response['message']));
				$this->buat_log("{$this->xendit_status} Order Created: Xendit was sent error response..", false);
				if($this->developmentmode == 'yes')
					$this->buat_log(json_encode($response, JSON_PRETTY_PRINT), false);
				return;
			
			}
			
			if($response['status'] == 'PAID' || $response['status'] == 'COMPLETED'){
				$this->buat_log("{$this->xendit_status} Order Created: Order #{$order->id} is coplete no future action required.", false);
				return;
			}
		...
		public function receipt_page( $order_id ){
			...
			if(isset($data['error_code']) || (NULL == $invoice && $invoice_exp < time())){
				...
				$this->buat_log($this->xendit_status .' '.$message, true);
				echo '
				<script>
	...
	//				$response = $this->xenditClass->getInvoice($invoice);
		...
		public function validate_payment($response){
			...
			if(/*in_array($response->merchant_name, $merchant_names) && */$order_id != '') {
				...
				$this->buat_log("{$xendit_status} Xendit merchant check passed, continue!", true);
				
				if($response->id == '579c8d61f23fa4ca35e52da4'){
					$this->buat_log("{$xendit_status} Invoice #{$response->id} was pushed from Xendit!", true);
				...
				if($this->developmentmode != 'yes'){
					...
					if ( FALSE === get_post_status( $order->id ) || ($payment_gateway->id != $this->id)) {
						...
						$this->buat_log("{$xendit_status} Xendit is live and required valid order id!", false);
				...
				if($this->sucess_responce_xendit == $response->status || 'PAID' == $response->status){
					...
					$this->buat_log("{$xendit_status} Xendit is {$response->status}, Proccess Order!", true);
					...
					$this->buat_log("{$xendit_status} Order #{$order->id} now mark as complete with Xendit!", true);
					...
					$this->buat_log("{$xendit_status} Xendit is {$response->status}, Proccess Order Declined!", false);
				...
				//$this->buat_log("{$xendit_status} Xendit merchant check not passed, break!", false);
				$this->buat_log("{$xendit_status} Xendit external id check not passed, break!", false);
		...
		public function buat_log( $message, $success, $end = false ){
	...
	function check_xendit_response(){
		...
		if(isset($_REQUEST['xendit_mode']) && $_REQUEST['xendit_mode'] == 'xendit_callback'){
			...
			if( ($_SERVER["REQUEST_METHOD"] === "POST") && ($_SERVER["SCRIPT_NAME"] === "{$script_base}/index.php") ){
				...
				if($headers['X-Callback-Token'] == $xendit->callback_token){
					$data = file_get_contents("php://input");
					$response = json_decode($data);

					$xendit->buat_log("{$xendit_status} Callback Request: Xendit was sent valid data!", true);
				
					if($xendit->developmentmode == 'yes')
						$xendit->buat_log(json_encode($response, JSON_PRETTY_PRINT), true);

					$xendit->validate_payment($response);
				}
				else {
					$xendit->buat_log("{$xendit_status} Callback Request: Invalid token!", true);
					header('HTTP/1.1 501 Invalid Token');
					exit;
				}
			}
			else {
				$xendit->buat_log("{$xendit_status} Callback Request: Invalid callback!", true);
