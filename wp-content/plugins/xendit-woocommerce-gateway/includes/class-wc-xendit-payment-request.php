 ...
 *
 * @package WooCommerce_Xendit/Classes/Payment_Request
 * @since   3.1.0
 * @version 3.1.0
 */
...
/**
 * WC_Xendit_Payment_Request class.
 */
class WC_Xendit_Payment_Request {
	...
	public function __construct() {
		...
		add_action( 'wc_ajax_wc_xendit_get_cart_details', array( $this, 'ajax_get_cart_details' ) );
		add_action( 'wc_ajax_wc_xendit_get_shipping_options', array( $this, 'ajax_get_shipping_options' ) );
		add_action( 'wc_ajax_wc_xendit_update_shipping_method', array( $this, 'ajax_update_shipping_method' ) );
		add_action( 'wc_ajax_wc_xendit_create_order', array( $this, 'ajax_create_order' ) );
	}

	/**
	 * Check if Xendit gateway is enabled.
	 *
	 * @return bool
	 */
	protected function is_activated() {
		$options             = get_option( 'woocommerce_xendit_settings', array() );
		$enabled             = isset( $options['enabled'] ) && 'yes' === $options['enabled'];
		$xendit_checkout     = isset( $options['xendit_checkout'] ) && 'yes' !== $options['xendit_checkout'];
		$request_payment_api = isset( $options['request_payment_api'] ) && 'yes' === $options['request_payment_api'];

		return $enabled && $xendit_checkout && $request_payment_api && is_ssl();
	}
	...
	public function scripts() {
		...
		wp_enqueue_script( 'xendit', 'https://js.xendit.co/v1/xendit.min.js', '', '1.0', true );
		wp_enqueue_script( 'wc-xendit-payment-request', plugins_url( 'assets/js/payment-request' . $suffix . '.js', WC_XENDIT_MAIN_FILE ), array( 'jquery', 'xendit' ), WC_XENDIT_VERSION, true );

		wp_localize_script(
			'wc-xendit-payment-request',
			'wcXenditPaymentRequestParams',
			array(
				'ajax_url' => WC_AJAX::get_endpoint( '%%endpoint%%' ),
				'xendit'   => array(
					'key'                => $this->get_publishable_key(),
					'allow_prepaid_card' => apply_filters( 'wc_xendit_allow_prepaid_card', true ) ? 'yes' : 'no',
				),
				'nonce'    => array(
					'payment'         => wp_create_nonce( 'wc-xendit-payment-request' ),
					'shipping'        => wp_create_nonce( 'wc-xendit-payment-request-shipping' ),
					'update_shipping' => wp_create_nonce( 'wc-xendit-update-shipping-method' ),
	...
	public function ajax_get_cart_details() {
		check_ajax_referer( 'wc-xendit-payment-request', 'security' );
		...
		$data = array(
			...
			'order_data'        => array(
				'total' => array(
					'label'  => __( 'Total', 'woocommerce-gateway-xendit' ),
					'amount' => array(
	...
	public function ajax_get_shipping_options() {
		check_ajax_referer( 'wc-xendit-payment-request-shipping', 'security' );
	...
	public function ajax_update_shipping_method() {
		check_ajax_referer( 'wc-xendit-update-shipping-method', 'security' );
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'woocommerce-gateway-xendit' ) );
		}
	...
	protected function compute_display_items( $shipping_method_id ) {
		...
		$items = array(
			...
			array(
				'label' => __( 'Subtotal', 'woocommerce-gateway-xendit' ),
				'amount' => array(
		...
		if ( 0 < $tax_total ) {
			$items[] = array(
				'label'  => __( 'Tax', 'woocommerce-gateway-xendit' ),
				'amount' => array(
					'currency' => $currency,
					'value'    => $tax_total,
				),
