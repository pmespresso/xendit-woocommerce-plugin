/*global jQuery, wcXenditPaymentRequestParams, PaymentRequest, Xendit, Promise */
/*jshint es3: false */
/*jshint devel: true */
(function( $ ) {

	/**
	 * WooCommerce Xendit PaymentRequest class.
	 *
	 * @type {Object}
	 */
	var wcXenditPaymentRequest = {
		...
		getAjaxURL: function( endpoint ) {
			return wcXenditPaymentRequestParams.ajax_url
				.toString()
				.replace( '%%endpoint%%', 'wc_xendit_' + endpoint );
		},
		...
		initPaymentRequest: function( evt ) {
			evt.preventDefault();
			var self = wcXenditPaymentRequest;
			var data = {
				security: wcXenditPaymentRequestParams.nonce.payment
			};
		...
		updateShippingOptions: function( details, address, resolve, reject ) {
			var self = this;
			var data = {
				security:  wcXenditPaymentRequestParams.nonce.shipping,
		...
		updateShippingDetails: function( details, shippingOption, resolve, reject ) {
			...
			var selected = null;
			var data     = {
				security:  wcXenditPaymentRequestParams.nonce.update_shipping,
				shipping_method: [
					shippingOption
				]
			};
			...
			$.ajax({
				...
				success: function( response ) {
					...
					if ( null === selected ) {
						reject( wcXenditPaymentRequestParams.i18n.unknown_shipping.toString().replace( '[option]', shippingOption ) );
					}
		...
		getOrderData: function( payment ) {
			...
			var data     = {
				_wpnonce:                  wcXenditPaymentRequestParams.nonce.checkout,
		...
		processPayment: function( payment ) {
			...
			Xendit.setPublishableKey( wcXenditPaymentRequestParams.xendit.key );
			Xendit.createToken( cardData, function( status, response ) {
				if ( response.error ) {
					self.abortPayment( payment, self.getErrorMessageHTML( response.error.message ) );
				} else {
					// Check if we allow prepaid cards.
					if ( 'no' === wcXenditPaymentRequestParams.xendit.allow_prepaid_card && 'prepaid' === response.card.funding ) {
						self.abortPayment( payment, self.getErrorMessageHTML( wcXenditPaymentRequestParams.i18n.no_prepaid_card ) );
					} else {
						// Token contains id, last4, and card type.
						orderData.xendit_token = response.id;
